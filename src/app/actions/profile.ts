...
export async function getProfileData(): Promise<ProfileData | null> {
  ...
  const activityIds = [...recentReviews.map(r => r.perfumeId), ...recentWardrobe.map(w => w.perfumeId)];
  ...
  const fields = [
    ...
    { key: 'favNotes', label: 'Fav Notes', isFilled: userRecord.favNotes.length > 0 },
    { key: 'favPerfumes', label: 'Top 5 Perfumes', isFilled: userRecord.favPerfumeIds.length > 0 },
    { key: 'favAccords', label: 'Fav Accords', isFilled: userRecord.favAccords.length > 0 },
    { key: 'favPerfumers', label: 'Fav Perfumers', isFilled: userRecord.favPerfumers.length > 0 }
  ];
  
  const missingFields = fields.filter(f => !f.isFilled).map(f => f.label);
  const completionPercentage = Math.round((fields.filter(f => f.isFilled).length / fields.length) * 100);
...
export async function updateProfile(data: { 
  ...
  const toArray = (str: string) => str.split(',').map(s => s.trim()).filter(s => s.length > 0);
